...
from abc_cook.schema import CookingGraph, CookingPlan, LongWait, PlanSummary, Session
...
def _elapsed_high_min(graph: CookingGraph, *, burner_capacity: int) -> float:
    ...
    return schedule(slowed_graph, burner_capacity=burner_capacity).total_min
...
def summarize(graph: CookingGraph, plan: CookingPlan, *, burner_capacity: int = 1) -> PlanSummary:
    ...
    for i, cluster in enumerate(clusters):
        session_active = float(
            sum(
                ...
                for node_id in node_ids_by_cluster[i]
    ...
    long_waits = [
        LongWait(
            ...
            host_node_ids=[
                s.node_id
                for s in by_start
                if not s.occupies_cook and s.start_min < end and s.end_min > start
            ],
        )
    ...
    return PlanSummary(
        ...
        elapsed_high_min=_round(_elapsed_high_min(graph, burner_capacity=burner_capacity)),
        sessions=sessions,
