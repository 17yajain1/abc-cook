"""Combines acquisition legs 1+2 into one `RawAcquisition`. No LLM.
...
def acquire(url: str) -> RawAcquisition:
    """Leg 1 (YouTube metadata), then leg 2 (first description link with Recipe JSON-LD).

    Leg 2 is a bonus and fails soft by construction (`blog.fetch_recipe` never raises):
    if no candidate link resolves to a Recipe, `blog_recipe` stays None and the
    description-only result from leg 1 is returned unchanged.
    """
